 ...
 */
public class LeetCode_547_202 {
    public static int findCircleNum(int[][] M) {
        int[] visited = new int[M.length];
        int result = 0;
        for (int i = 0; i < M.length; i++) {
            if (visited[i] == 0) {
                recursive(M, visited, i);
                result++;
            }
        }
        return result;
    ...
    private static void recursive(int[][] M, int[] visited, int i) {
            for (int j = 0; j < M.length; j++) {
                if (M[i][j] == 1 && visited[j] == 0) {
                    visited[j] = 1;
                    recursive(M, visited, j);
                }
            }
        }
...
//        ]
        int[][] M = new int[][]{{1, 1, 0},
                {1, 1, 1},
                {0, 1, 1}};
        System.out.println(LeetCode_547_202.findCircleNum(M));
    }
}
