 ...
 */
const trap: (height: number[]) => number = (height: number[]) => {
  const stack: number[] = []; // 用于存储左边界的索引
  let area: number = 0;

  for (let index = 0; index < height.length; index++) {
    while (stack.length && height[stack[stack.length - 1]] < height[index]) {
      // 如果栈顶的值，小于当前值，则表示遇到了右边界，需要计算面积
      const subface = stack.pop(); // 栈顶作为底部的高度

      if (stack.length) {
        // 若进行出栈操作之后，栈仍有值，则说明可以计算面积
        const leftHeight = height[stack[stack.length - 1]]; // 此时的栈顶高度，作为计算面积的左边高度
        const rightHeight = height[index]; // 此时的元素高度，作为计算面积的右边高度
        const realHeight = Math.min(leftHeight, rightHeight) - height[subface]; // 左右高度取最小值后，减去底部高度，则为当前需要计算面积区域的实际高度
        const realWidth = index - stack[stack.length - 1] - 1; // 当前元素索引减去当前栈顶索引，再减1，则为计算面积的宽度
